...
import type { MonthlyConsumptionStatsData } from "@/lib/api"
import {
  ResponsiveContainer,
  ComposedChart,
  Bar,
...
interface BackupConsumptionChartProps {
  data: MonthlyConsumptionStatsData[]
}
...
export function BackupConsumptionChart({ data }: BackupConsumptionChartProps) {
  const { t } = useLanguage()
    ...
    <div className="mb-4 sm:mb-6 space-y-3 sm:space-y-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-3 sm:p-4 shadow-sm">
        <div className="mb-3 sm:mb-4">
          <h3 className="text-sm sm:text-base font-semibold text-gray-900 dark:text-gray-100">{t("备用号池月度额度消耗", "Backup Pool Monthly Consumption")}</h3>
          ...
          <div className="h-[280px] sm:h-[360px]">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartDisplayData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                ...
                <Bar yAxisId="left" dataKey="totalConsumption" fill="#f97316" name={t("额度消耗", "Consumption")} radius={[4, 4, 0, 0]} />
                <Line yAxisId="right" type="monotone" dataKey="growthRateForChart" stroke="#eab308" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} name={t("增长率", "Growth")} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        )}
